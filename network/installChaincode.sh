#!/usr/bin/env bash
docker exec cli peer chaincode install -n universityCertificate -v 0 -p /opt/gopath/src/github.com/universityCertificate/node -l "node"

docker exec cli peer chaincode instantiate -n universityCertificate -v 0 -C mychannel -l "node" -c '{"Args":[]}'

docker exec cli peer chaincode invoke -C mychannel -n universityCertificate -c '{"Args":["createCertificateProposal", "certProposal1", "public_key","file_hash","org2.example.com"]}'
...
docker exec cli peer chaincode query -C mychannel -n universityCertificate -c '{"Args":["queryCertificateProposals"]}'
